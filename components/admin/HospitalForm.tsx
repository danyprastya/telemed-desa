...
export function HospitalForm({ onSuccess }: HospitalFormProps) {
  ...
  const onSubmit = async (data: InstitutionInput) => {
    ...
    try {
      ...
      toast.error('Gagal membuat rumah sakit')
    } finally {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="max-w-md">
      <Card className="border-border-green">
        <CardHeader>
          <CardTitle className="text-lg">Rumah Sakit Baru</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="name">Nama Rumah Sakit</Label>
            <Input id="name" {...register('name')} placeholder="Nama rumah sakit" disabled={isSubmitting} />
