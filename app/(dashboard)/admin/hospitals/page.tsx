...
import type { Hospital } from '@/types/app.types'
...
export default function HospitalsPage() {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      toast.error('Gagal menambahkan rumah sakit')
    } finally {
      setSubmitting(false)
    }
  }
  ...
  return (
    <RoleGuard allowedRoles={['admin']}>
        ...
        <DialogContent>
          ...
          <div className="space-y-4 pt-2">
            <div className="space-y-2">
              ...
              <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="RSUD Kota Harapan" />
            </div>
            <div className="space-y-2">
              <Label>Lokasi</Label>
              <Input value={location} onChange={(e) => setLocation(e.target.value)} placeholder="Kota Harapan, Provinsi..." />
            </div>
      ...
      {loading ? <LoadingSpinner /> : items.length === 0 ? (
        <EmptyState title="Belum ada rumah sakit" description="Tambahkan rumah sakit pertama" />
      ) : (
              ...
              {items.map((h) => (
                <TableRow key={h.id}>
                  ...
                  <TableCell className="text-text-secondary">
                    <div className="flex items-center gap-1"><MapPin className="h-3 w-3" /> {h.location}</div>
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
